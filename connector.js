 ...
 *
 * @param {string} serviceNowConnector - The table target of the ServiceNow table API.
 * @param {string} [query] - Optional URL query string.
 *
 * @return {string} ServiceNow URL
 */
function constructUri(serviceNowConnector, query = null) {
  let uri = `/api/now/table/${serviceNowConnector}`;
  if (query) {
    uri = uri + '?' + query;
  }
  return uri;
 ...
 * @param {string} callOptions.serviceConnector - The table target of the ServiceNow table API.
...
function sendRequest(callOptions, callback) {
  ...
  if (callOptions.query)
    uri = constructUri(callOptions.serviceNowConnector, callOptions.query);
  else
    uri = constructUri(callOptions.serviceNowConnector);
  /**
 ...
 *
 * @param {object} callOptions - Passed call options.
 * @param {string} callOptions.serviceNowConnector - The table target of the ServiceNow table API.
 * @param {iapCallback} callback - Callback a function.
 * @param {(object|string)} callback.data - The API's response. Will be an object if sunnyday path.
 *   Will be HTML text if hibernating instance.
 * @param {error} callback.error - The error property of callback.
